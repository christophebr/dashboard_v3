#!/usr/bin/env python3
"""
Script final pour créer des modèles vraiment différents avec descriptions enrichies
"""
...
def creer_modele_classique():
    """Crée un modèle classique avec descriptions originales"""
...
def creer_modele_enrichi():
    """Crée un modèle enrichi avec descriptions enrichies"""
    print("\n🤖 CRÉATION DU MODÈLE ENRICHİ")
    print("=" * 50)
    
    # Charger les données enrichies
    df = pd.read_excel('data/Affid/modele/modele_avec_definitions.xlsx')
    df = df.dropna(subset=['catégorie', 'description_enrichie'])
    ...
    # Utiliser les descriptions enrichies
    X_text = df['description_enrichie'].apply(preprocess_text)
    y_categories = df['catégorie']
    ...
    print(f"   Descriptions enrichies: {len(df)} exemples")
...
def tester_predictions(vectorizer_classic, classifier_classic, vectorizer_enriched, classifier_enriched):
    ...
    for i, example in enumerate(test_examples, 1):
        ...
        # Prédiction enrichie (avec description enrichie)
        # Créer une description enrichie simple pour le test
        example_enriched = f"Description: {example}"
        example_tfidf_enriched = vectorizer_enriched.transform([example_enriched])
...
def main():
    """Fonction principale"""
    print("🚀 CRÉATION DE MODÈLES FINAUX AVEC DESCRIPTIONS ENRICHIES")
    print("=" * 70)
