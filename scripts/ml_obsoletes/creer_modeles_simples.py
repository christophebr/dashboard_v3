#!/usr/bin/env python3
"""
Script simple pour créer deux modèles différents à partir du même fichier
"""
...
def creer_modele_enrichi():
    """Crée un modèle enrichi avec paramètres optimisés"""
    print("\n🤖 CRÉATION DU MODÈLE ENRICHİ")
    print("=" * 50)
    
    # Charger les mêmes données mais avec des paramètres différents
    df = pd.read_excel('data/Affid/modele/modele.xlsx')
    df = df.dropna(subset=['catégorie', 'description'])
...
def main():
    """Fonction principale"""
    print("🚀 CRÉATION DE MODÈLES DIFFÉRENTS")
    print("=" * 60)
    ...
    has_differences = tester_predictions(vectorizer_classic, classifier_classic, vectorizer_enriched, classifier_enriched)
    
    if has_differences:
        print("\n✅ Différences détectées entre les modèles !")
        ...
        print("\n🎉 SUCCÈS ! Les modèles sont maintenant différents.")
        ...
        print("\n❌ Aucune différence détectée. Les modèles sont identiques.")
        print("💡 Suggestions:")
        print("   - Augmenter les différences de paramètres")
        print("   - Utiliser des algorithmes différents")
        print("   - Modifier les données d'entraînement")
