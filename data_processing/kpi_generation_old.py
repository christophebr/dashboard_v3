...
def charge_ticket(df_ticket, agents):
    ...
    df_filtered = df_ticket[
        ...
        (df_ticket['Pipeline'].isin(['SSIA', 'SSI'])) &
...
def graph_taux_jour (df_support): 
    ...
    fig.update_yaxes(ra=[20,120], secondary_y=True)
...
def compute_ticket_appels_metrics(df_tickets, df_support, agent):
    ...
    df_filtered_pipeline = df_tickets[
        (df_tickets['Pipeline'].isin(['SSI', 'SSIA'])) &
        (df_tickets['Source'].isin(['Chat', 'E-mail', 'Formulaire']) | pd.isna(df_tickets['Source']))
    ]
    ...
    moyenne_tickets_total = df_filtered_agent.groupby(['Date'])['Ticket ID'].nunique().mean() if not df_tickets.empty else 0
    ...
    df_ticket_ref = df_filtered_pipeline.groupby(['Date', 'Propriétaire du ticket']).size().reset_index(name='TotalTickets')
    df_ticket_std = df_ticket_ref['TotalTickets'].std() if not df_ticket_ref.empty else 0
    df_ticket_mean = df_ticket_ref['TotalTickets'].mean() if not df_ticket_ref.empty else 0
...
def historique_scores_total(agents_n1, df_tickets, df_support, date_debut=None, nb_semaines=8):
    ...
    # Si aucune date de début n'est fournie, on prend la date maximale dans df_tickets
    if date_debut is None:
        max_date = pd.to_datetime(df_tickets['Date']).max().normalize()
    else:
        max_date = pd.to_datetime(date_debut).normalize()

    # Pour chaque semaine, on ajuste la date à la semaine correspondante
    for i in range(nb_semaines):
        # Calcul de la date de début et fin de la semaine
        start_of_week = max_date - pd.Timedelta(weeks=i)
        # Trouver le début de la semaine (lundi) pour la période spécifiée
        start_of_week = start_of_week - pd.Timedelta(days=start_of_week.weekday())
        end_of_week = start_of_week + pd.Timedelta(days=6)

        # Formater la semaine sous la forme 'SYYYY-Www'
        week_label = start_of_week.strftime('S%Y-%U')

        # Filtrer les données de tickets et de support pour la semaine
        ...
        # Si aucune donnée n'existe pour cette semaine, on passe à la suivante
        if df_tickets_semaine.empty or df_support_semaine.empty:
            continue

        # Calcul des KPIs pour cette semaine
        ...
        df_week['score total'] = (df_week['score ticket'] + df_week['score appel']) / 2
        df_week['Semaine'] = week_label  # Utilisation de la semaine comme date

        historique.append(df_week)

    # Si l'historique est vide (aucune donnée valide), on renvoie un message d'erreur
    if not historique:
    ...
    # Tri des semaines pour l'affichage
...
def df_compute_ticket_appels_metrics(agents_n1, df_ticket, df_support):
    ...
    for agent in agents_n1:
        ...
        rows.append({
            ...
            "% tickets": round(kpis.get('moy_ticket_agent', 0) / (kpis.get('nb_appels_jour', 0) + kpis.get('moy_ticket_agent', 0)),2) *100,
            "% appels": round(kpis.get('nb_appels_jour', 0) / (kpis.get('nb_appels_jour', 0) + kpis.get('moy_ticket_agent', 0)),2) * 100,
            '% appel entrant agent': round(kpis.get('ratio_entrants', 0),2) *100,
            'ref_ticket_agent':round(kpis['ref_ticket_agent'],2),
            'ref_appel_agent':round(kpis['ref_appel_agent'],2),
        })
...
def evo_appels_ticket(df_ticket, df_support):
    ...
    return fig, activite_appels, activite_tickets
...
def generate_kpis(df_support, df_tickets, agents, partenaire=None, partenaires=None):
    ...
    evo_appels_tickets, activite_appels_pourcentage,  activite_tickets_pourcentage= evo_appels_ticket(df_tickets, df_support)
